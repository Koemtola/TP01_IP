...
<template>
  <main>
    ...
    <div class="bg-gray-500 py-2 text-lg text-center  relative" style="color: rgb(0, 255, 0);">
      TopOne.com
      <div class="
          absolute
          text-white
    ...
    <div class="app">
      <div class="flex flex-row  ml-2 pt-1">
        <section class="flex flex-col p-2  bg-gray-200 mt-1">
          <div class="ctg-box pl-2" v-for="cate in categories" :key="cate.name">
            <div>
              <strong class=" font-medium text-dark">✨ {{ cate.name }}</strong>
            </div>
            <ul>
              <li v-for="item in cate.items" :key="item.name" @click="getProductByItem(item)"
        ...
        <section class="sec-right flex flex-col p-2 space-y-2" style="border: none;">
          <div class="">
            ...
            <div class="box " v-for="product in products" :key="product.title" style="border: none;">
